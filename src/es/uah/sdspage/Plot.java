    ...
    implements Runnable
{
    ...
    {
        if(showNotBracketed)
            g.drawString("La movilidad relativa no est� comprendida entre los patrones", xArray[leftGridCol], size().height / 8);
    }
    ...
    private void showSampMW(Graphics g)
    {
        ...
        if(questionRCorr)
        {
            g.drawString("No coincide. La movilidad est� bien, quiz�s la recta ajuste mal", xArray[leftGridCol], getSize().height / 6);
            if(!bongPlayed)
            {
                parent.playBong();
                bongPlayed = true;
